...
import math
import shutil
from .sLog import slogPrint
from . import Renamer
from . import Archiver_TS
...
def executeStickerSheetGenerator() :
    ...
    slogPrint(' ---- Beginning Sticker Sheet Generation ---- ')
    ...
    if params['addCutLineToTicket'] == True:
        for i in allInfoPDFs :
            ...
            slogPrint(' - Added cut line and barcode to infotech for order ' + itemSpecs["order"] + ' | SKU ' + itemSpecs["sku"] + ' succesfully.')
    ...
    for i in allPrintPDFs :
        ...
        if params['DontIncludeTicket'] == True:
            ...
            try:
                ...
                slogPrint(' - No Infotech found for ' + itemSpecs["order"] + ' | SKU ' + itemSpecs["sku"])

        pythonSheetGenerator(i, infotech, itemSpecs, params)

    slogPrint('\n ---- Sticker sheet generation complete! ---- ')
...
def pythonSheetGenerator(printFileName, ticketFileName, itemSpecs, params) :
    ...
    for i in range(sheetsNeeded) :
        ...
        slogPrint(' ---- Creating sheet ' + str(sheetIndex) + ' of ' + str(sheetsNeeded) + ' for order ' + str(itemSpecs["order"]) + ' | SKU ' + str(itemSpecs["sku"]))
