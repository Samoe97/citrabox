...
def ItemBuilderFileID(fileID) :
    ...
    for j in orderData["orderData"]["items"] :
        ...
        if j["components"][0]["_id"] == currentItem['componentId']:
            ...
            elif j['components'][0]['attributes']['substrate'] == 'Magnet' :
                    productType = 'Magnet'
                    sku = j["components"][0]["attributes"]["Customer_SKU"]
            else :
                productType = 'Sticker'
...
def MakeNameFromItem(item, backLabel):
    ...
    #     newFile = directory + '/' + str(item.order) + '_' + str(item.sku) + options + '_' + str(item.width) + 'x' + str(item.height) + '_qty ' + str(item.quantity) + '_' + backLabel + '.pdf'
    # elif item.type == 'Tile' :
    #     newFile = directory + '/TILE_' + str(item.order) + '_' + str(item.sku) + options + '_qty ' + str(item.quantity) + '_' + backLabel + '.pdf'
    # elif item.type == 'Tile Distressed' :
    #     newFile = directory + '/TILE DISTRESSED_' + str(item.order) + '_' + str(item.sku) + '_qty ' + str(item.quantity) + '_' + backLabel + '.pdf'
    # elif item.type == 'Magnet' :
    #     newFile = directory + '/' + str(item.order) + '_' + str(item.sku) + options + '_Magnet' + '_' + str(item.width) + 'x' + str(item.height) + '_qty ' + str(item.quantity) + '_' + backLabel + '.pdf'
    # elif item.type == 'Cap' :
    #     newFile = directory + '/CAPS 50up_' + str(item.order) + options + '_qty ' + str(item.quantity) + '_' + backLabel + '.pdf'
    # elif item.type == 'MetalRound' :
    #     newFile = directory + '/ROUND_' + str(item.order) + '_' + str(item.sku) + '_qty ' + str(item.quantity) + '_' + backLabel + '.pdf'
    # elif item.type == 'MetalLandscape' :
    #     newFile = directory + '/METAL_' + str(item.order) + '_' + str(item.sku) + '_qty ' + str(item.quantity) + '_13x11.5_' + backLabel + '.pdf'
...
def ParseFolder(params) :
    ...
    RenameItems(totalItems, directory)

def RenameItems(totalItems, directory) :
    # RENAME PRINT AND TICKET FILES
    for i in totalItems :
        for file in os.listdir(directory):
            if file.endswith('.pdf') :
                if file.find(i.barcode) != -1 :
                    ...
                    if i.type == 'Sticker' :
                    ...
                    elif i.type == 'Tile Distressed' :
                        newFile = directory + '/TILE DISTRESSED_' + str(i.order) + '_' + str(i.sku) + '_qty ' + str(i.quantity) + '_' + backLabel + '.pdf'
                    elif i.type == 'Magnet' :
                        ...
                        newFile = directory + '/CAPS 50up_' + str(i.order) + options + '_qty ' + str(i.quantity) + '_' + backLabel + '.pdf'
                    ...
                    sheetIndex = 1
                    for file in os.listdir(directory) :
                        if directory + '/' + file == newFile :
                            found = True
                            if file.__contains__('-') :
                                splitfile = file.split('-')
                                newFile = directory + '/' + splitfile[0] + '-' + str(sheetIndex) + '.pdf'
                                sheetIndex = sheetIndex + 1
                                continue
                            else:
                                newFile = newFile[:-4] + '-' + str(sheetIndex) + '.pdf'
                                sheetIndex = sheetIndex + 1
                                continue

                    if os.path.exists(newFile) == True:
                        sheetIndex = sheetIndex + 1
                        newFile = newFile[:-6] + '-' + str(sheetIndex) + '.pdf'
                        if os.path.exists(newFile) == True:
                            sheetIndex = sheetIndex + 1
                            newFile = newFile[:-6] + '-' + str(sheetIndex) + '.pdf'
                            if os.path.exists(newFile) == True:
